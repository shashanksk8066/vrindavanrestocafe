...
import { Calendar, Clock, Phone, MapPin, Download, Truck } from 'lucide-react';
import { generateNumericId } from '../../utils/formatId';

const AdminSubscriptionBookings = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const bookingsSnap = await getDocs(collection(db, 'subscriptionBookings'));
            const bookingsData = [];
            ...
            const menusData = { ...menuItemsMap };
            menusSnap.forEach(docSnap => {
                menusData[docSnap.id] = docSnap.data();
            });
            const mainMenusSnap = await getDocs(collection(db, 'mainMenu'));
            mainMenusSnap.forEach(docSnap => {
                menusData[docSnap.id] = docSnap.data();
            });
            setMenuItemsMap(menusData);
    ...
    const handleAssignDelivery = async () => {
        ...
        try {
            for (let bookingId of selectedBookings) {
                await updateDoc(doc(db, 'subscriptionBookings', bookingId), {
        ...
        pendingBookings.forEach(booking => {
            if (booking.menuItems && Array.isArray(booking.menuItems)) {
                booking.menuItems.forEach(item => {
                    ...
                    const itemName = menuItemsMap[itemId]?.name || 'Unknown Item';
                    itemCounts[itemName] = (itemCounts[itemName] || 0) + qty;
                });
            }
            if (booking.addOnItems && Array.isArray(booking.addOnItems)) {
                booking.addOnItems.forEach(item => {
                    const itemId = typeof item === 'string' ? item : item.id;
                    const qty = typeof item === 'string' ? 1 : (item.quantity || 1);
                    const itemName = menuItemsMap[itemId]?.name || 'Unknown Add-on';
                    itemCounts[itemName] = (itemCounts[itemName] || 0) + qty;
                });
            }
        });
        ...
        pendingBookings.forEach((booking, index) => {
            ...
            const slot = typeof booking.deliverySlot === 'string' ? booking.deliverySlot : (booking.deliverySlot ? `${booking.deliverySlot.name || ''} ${booking.deliverySlot.start || ''} ${booking.deliverySlot.end || ''}` : 'No Slot');
            ...
            doc.setFont('helvetica', 'normal');
            doc.text(`Slot:`, 20, yPos + 26 + addressHeight);
            doc.setFont('helvetica', 'bold');
            doc.text(slot, 45, yPos + 26 + addressHeight);
            ...
            if (booking.menuItems) {
                booking.menuItems.forEach(i => {
                    const id = typeof i === 'string' ? i : i.id;
                    const qty = typeof i === 'string' ? 1 : (i.quantity || 1);
                    const name = menuItemsMap[id]?.name || 'Unknown Item';
                    doc.text(`- ${name} (x${qty})`, pageWidth / 2 + 10, itemY);
                    itemY += 6;
                });
            }
            if (booking.addOnItems) {
                booking.addOnItems.forEach(i => {
                    const id = typeof i === 'string' ? i : i.id;
                    const qty = typeof i === 'string' ? 1 : (i.quantity || 1);
                    const name = menuItemsMap[id]?.name || 'Unknown Add-on';
                    doc.text(`- [Add-on] ${name} (x${qty})`, pageWidth / 2 + 10, itemY);
                    itemY += 6;
                });
            }
        ...
        doc.save(`delivery_receipts_${selectedDate}.pdf`);
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <h1 className="text-2xl font-bold text-gray-900">Subscription Bookings</h1>
                ...
                <div className="p-6">
                    ...
                    {loading ? (
                        ...
                        <div className="py-24 bg-white rounded-3xl border border-gray-100 text-center shadow-sm">
                            ...
                            <p className="text-gray-500 mt-2">There are no {activeTab} bookings for this date.</p>
                        </div>
                    ) : (
                        <div>
                            {activeTab === 'pending' && (
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="p-6 flex-1 flex flex-col">
                                                        ...
                                                        {booking.menuItems?.map((item, i) => {
                                                            ...
                                                            return (
                                                                <div key={i} className="flex justify-between items-center text-sm">
                                                                    ...
                                                                    <span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded font-bold text-xs">x{qty}</span>
                                                                </div>
                                                            );
                                                        })}
                                                        {booking.addOnItems?.map((item, i) => {
                                                            const id = typeof item === 'string' ? item : item.id;
                                                            const qty = typeof item === 'string' ? 1 : (item.quantity || 1);
                                                            const name = menuItemsMap[id]?.name || (item.name || 'Unknown Add-on');
                                                            return (
                                                                <div key={i} className="flex justify-between items-center text-sm">
                                                                    <span className="font-medium text-gray-800 flex items-center">
                                                                    ...
                                                                    <div className="flex items-center space-x-2">
                                                                        <span className="text-[10px] font-bold text-amber-600 uppercase bg-amber-50 px-1.5 py-0.5 rounded">Add-on</span>
                                                                        <span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded font-bold text-xs">x{qty}</span>
                                                                    </div>
                                                                </div>
                                                            );
                                                ...
                                                {/* Delivery Slot Footer */}
                                                <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                                                    {booking.deliverySlot ? (
                                                        <div className="inline-flex items-center px-3 py-1.5 rounded-xl bg-gray-900 text-white text-xs font-semibold shadow-md">
                                                            <Clock className="w-3.5 h-3.5 mr-1.5 text-amber-500" />
                                                            {typeof booking.deliverySlot === 'string' ? booking.deliverySlot : `${booking.deliverySlot.name || ''} ${booking.deliverySlot.start || ''}`}
                                                        </div>
                                                    ) : (
                                                        <span className="text-gray-400 text-xs font-medium">No Slot Selected</span>
                                                    )}
                                                    ...
                                                    {booking.status === 'assigned' && booking.deliveryBoyId && (
                                                        <div className="flex items-center text-xs font-medium text-blue-600 bg-blue-50 px-2 py-1 rounded-lg border border-blue-100">
                                                            ...
                                                            {deliveryBoys.find(b => b.id === booking.deliveryBoyId)?.name?.split(' ')[0] || 'Unknown'}
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
...
export default AdminSubscriptionBookings;
